"""Persistence for profiles, orgs, playbooks, drafts, and analyses.

Uses Supabase (service role, bypasses RLS) when configured. Falls back to an
in-memory store for tests and local boots without credentials.
"""
...
class SupabaseStore:
    ...
    async def upsert_profile(self, user_id: str, email: str | None, full_name: str | None) -> dict[str, Any]:
        ...
        res = self._client.table("profiles").upsert(payload, on_conflict="user_id").execute()
        rows = res.data or []
    ...
    async def _seed(self, org_id: str) -> None:
        ...
        self._client.table("playbooks").upsert(payload, on_conflict="id").execute()
    ...
    async def create_playbook(self, org_id: str, user_id: str, row: dict[str, Any]) -> dict[str, Any]:
        ...
        res = self._client.table("playbooks").insert(payload).execute()
    ...
    async def save_analysis(
        ...
        self._client.table("analyses").upsert(
            {
                "id": analysis_id,
                "organization_id": org_id,
                "user_id": user_id,
    ...
    async def create_draft(self, org_id: str, user_id: str, row: dict[str, Any]) -> dict[str, Any]:
        ...
        res = self._client.table("drafts").insert(payload).execute()
        return (res.data or [payload])[0]
    ...
    async def save_reference(
        ...
        res = self._client.table("draft_references").insert(payload).execute()
        return (res.data or [payload])[0]
    ...
    async def get_references(self, org_id: str, ids: list[str]) -> list[dict[str, Any]]:
        if not ids:
            return []
        res = (
            self._client.table("draft_references")
            .select("*")
            .eq("organization_id", org_id)
            .in_("id", ids)
            .execute()
